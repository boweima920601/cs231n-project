...
class Model:
	...
	def run_model(self, session, dataset=None, epochs=1, batch_size=64, use_save=True, plot_losses=False, testing=False):
		if testing:
			...
			pool5_result = np.zeros((X_test.shape[0], 7 * 7 * 512))
			for i in range(int(math.ceil((X_test.shape[0] / batch_size)))):
				...
				# pool5_out = temp[1]
				pool5_result[start_idx: start_idx + batch_size, :] = temp[1]
				print('finished {}'.format(start_idx + batch_size))
			print ('Start knn on pool5...')
			pool5_knn = np.zeros((X_test.shape[0], 10))
			weight = np.linspace(1, 0.1, 10)
			for i in range(X_test.shape[0]):
				if i % 1000 == 0:
					print ('i %d' % i)
				knn_10 = np.argsort(np.sum(np.square(pool5_result - pool5_result[i]), axis = 1))[:10]
				pool5_knn[i] = [i] + [k for k in knn_10 if k != i]
			output_y_average = np.zeors_like(output_y)
			for i in range(X_test.shape[0]):
				output_y_average[i] = np.average(output_y[pool5_knn[i]], axis = 0, weights = weight)
			print ('Finished knn on pool5.')

			rows = np.load('../data/test_data_id.npy')
			cols = ['c0', 'c1', 'c2', 'c3','c4', 'c5', 'c6', 'c7', 'c8', 'c9']
			# result = pd.DataFrame(output_y, index = rows, columns = cols)
			result = pd.DataFrame(output_y_average, index = rows, columns = cols)
